...
describe("Battery Date Decoder", () => {
  ...
  it("can format a Bulldog serial number", () => {
    ...
    assert.equal(
      "Manufactured in Terrell, TX on October, 2008",
      decoder.getDateString()
    );
    ...
    assert.equal(
      "Manufactured in Wabash, IN on March, 2010",
      decoder.getDateString()
    );

    decoder = new Decoder("Bulldog", "S1020W137");
    assert.equal(
      "Manufactured in Wabash, IN on July, 2016",
      decoder.getDateString()
    );
  });
  ...
  it("can format an Douglas serial number", () => {
    ...
    assert.equal(
      "Manufactured in Monterrey, MX on October, 2013",
      decoder.getDateString()
    );
  ...
  it("can format an Enersys serial number", () => {
    ...
    assert.equal(
      "Manufactured in Richmond, KY on January, 2014",
      decoder.getDateString()
    );
  ...
  it("can format a GNB serial number", () => {
    let decoder1 = new Decoder("gnb", "KEM12345");
    assert.equal(
      "Manufactured in Kankakee on May, 2010",
      decoder1.getDateString()
    );

    let decoder2 = new Decoder("gnb", "GHG2867");
    assert.equal(
      "Manufactured in Kansas City on August, 2004",
      decoder2.getDateString()
    );

    let decoder3 = new Decoder("gnb", "GAY0055");
    assert.equal(
      "Manufactured in Kansas City on January, 2019",
      decoder3.getDateString()
    );
  });
  ...
  it("can format a Hawker serial number", () => {
    ...
    assert.equal(
      "Model: Powerline/Standard - Manufactured on April, 2006",
      decoder.getDateString()
    );

    decoder = new Decoder("HaWkEr", "EP104110646");
    assert.equal(
      "Model: Energy Plus/Standard - Manufactured on April, 2011",
      decoder.getDateString()
    );

    decoder = new Decoder("HAWKER", "EP311102180");
    assert.equal(
      "Model: Energy Plus/Lifespeed - Manufactured on November, 2010",
      decoder.getDateString()
    );

    decoder = new Decoder("HAWKER", "WL110182815");
    assert.equal(
      "Model: Water Less/Standard - Manufactured on October, 2018",
      decoder.getDateString()
    );
  });
